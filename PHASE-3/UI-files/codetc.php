...
<html lang="en">
<head>
  <title>Courier Customer Details</title>
  <meta charset="utf-8">
...
<div class="container-fluid text-center">    
  ...
  <h2 style="color:blue;text-decoration: underline;">Couriers</h2>
  <table>
<tr>
...
$sql="SELECT * FROM booking WHERE custid='{$_SESSION["cid"]}'";
    $result=mysqli_query($db,$sql) or die("Bad query:");
while($row=mysqli_fetch_assoc($result)){
 echo "<tr><td>{$row['consid']}</td><td>{$row['csender']}</td><td>{$row['creceiver']}</td><td>{$row['csource']}</td><td>{$row['cdest']}</td><td>{$row['camount']}</td><td>{$row['cdob']}</td></tr>"; 
}
